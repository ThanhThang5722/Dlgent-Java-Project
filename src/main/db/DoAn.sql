...
-- Xóa bảng liên quan đến đối tác, khách hàng và các dữ liệu liên quan
DROP TABLE LICH_SU_RUT_TIEN;
DROP TABLE KHACH_HANG;
DROP TABLE DOI_TAC;

-- Xóa bảng khu nghỉ dưỡng và các bảng liên quan
...
-- Xóa các bảng đặt phòng và giảm giá
...
-- Xóa các bảng liên quan đến phòng và thời gian phòng bán
...
BEGIN
    ...
    IF V_USERNAME IS NULL THEN
        RAISE_APPLICATION_ERROR(-20001, 'Không tìm thấy username từ token.');
    END IF;

    -- Lấy ID đối tác từ username
    BEGIN
        SELECT DT.ID INTO V_ID_DOI_TAC
        FROM DOI_TAC dt
        JOIN TAI_KHOAN tk ON DT.ID_TAI_KHOAN = TK.ACCOUNT_ID
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20002, 'Không tìm thấy đối tác với username: ' || V_USERNAME);
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20003, 'Lỗi khi truy vấn ID đối tác: ' || SQLERRM);
    END;

    -- Lấy ID thành phố từ tên thành phố
    BEGIN
        SELECT TP.ID INTO V_ID_THANH_PHO
        FROM THANH_PHO tp
        WHERE TP.TEN_THANH_PHO = P_THANH_PHO;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20004, 'Không tìm thấy thành phố với tên: ' || P_THANH_PHO);
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20005, 'Lỗi khi truy vấn ID thành phố: ' || SQLERRM);
    END;

    -- Lấy ID quận từ tên quận và thành phố
    BEGIN
        SELECT Q.ID INTO V_ID_QUAN
        FROM QUAN q
        WHERE Q.ID_THANH_PHO = V_ID_THANH_PHO
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20006, 'Không tìm thấy quận: ' || P_QUAN || ' ở thành phố: ' || P_THANH_PHO);
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20007, 'Lỗi khi truy vấn ID quận: ' || SQLERRM);
    END;

    -- Lấy ID phường từ tên phường và quận
    BEGIN
        SELECT P.ID INTO V_ID_PHUONG
        FROM PHUONG P
        WHERE P.ID_QUAN = V_ID_QUAN
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20008, 'Không tìm thấy phường: ' || P_PHUONG || ' ở quận: ' || P_QUAN);
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20009, 'Lỗi khi truy vấn ID phường: ' || SQLERRM);
    END;

    -- Thực hiện insert vào bảng KHU_NGHI_DUONG
    BEGIN
        INSERT INTO KHU_NGHI_DUONG (ID_DOI_TAC, TEN, DIA_CHI, PHUONG_ID)
        VALUES (V_ID_DOI_TAC, p_ten, p_dia_chi, V_ID_PHUONG);
    EXCEPTION
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20010, 'Lỗi khi thực hiện insert vào bảng KHU_NGHI_DUONG: ' || SQLERRM);
    END;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20011, 'Lỗi không xác định: ' || SQLERRM);
END;
/